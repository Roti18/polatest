...
def cmd_export(args):
    ...
    if top_n > 0:
        online = online[:top_n]
        label = f"top {top_n} fastest"
    else:
        label = "all online"

    # Kalo ada nama file, export (full atau top) ke file itu
    if out_file:
        export_data = []
        for r in online:
            export_data.append({
                ...
                "latency_ms": round(r.get("avg_ms") or r.get("ms") or 0, 1),
                "info": r.get("info", ""),
            })
        with open(out_file, "w") as f:
            json.dump(export_data, f, indent=2)
        print(f"[i] Exported {label} proxies to {out_file}")
        return

    print(f"\n{_DIM}{'PROXY':<22} {'PROTO':<6} {'COUNTRY':<22} {'LATENCY':<8} INFO{_RESET}")
    print(_DIM + _BAR * 80 + _RESET)
    for r in online:
        ms = f"{r.get('avg_ms') or r.get('ms') or 0:.0f} ms"
        country = r.get("country") or "-"
        print(f"{r['addr']:<22} {r['protocol']:<6} {country:<22} {ms:<8} {r['info']}")

    print(f"\n  {_GRN}Exported {len(online)} {label} proxies{_RESET}")
...
FLAG DETAILS:
  ...
  file (positional, export)
      Output filename for export results (JSON format).
      When provided, saves directly to that file instead of default CSV/working.
      Example: python main.py export hasil.json
               python main.py export hasil.json --top 5

OUTPUT FILES:
  results.json          Full test results with all metrics (auto-generated by test)
  results.csv           Same results in CSV format (auto-generated by export)
  proxies.working.json  Only online proxies, sorted by latency, ready to use (auto-generated by export)
...
EXAMPLES:
    ...
    python main.py test --limit 5              Test only first 5 proxies
    ...
    python main.py export                      Display + export to CSV/working
...
"""
    ...
    p_export.add_argument("file", nargs="?", default=None, help="Output filename (default: results.json). Example: export hasil.json")
    p_export.add_argument("--top", type=int, default=0, help="Only export the N fastest proxies. Generates CSV, and working.json.")
